...
**파일**: `figure6-1-research-design.svg` (또는 `.png`)

**캡션**:

```
그림 6.1 Design-Based Research(DBR) 기반 연구 설계 다이어그램

본 연구는 DBR 방법론에 따라 6단계로 진행되었다. 1단계에서 예비조사를 통해 문제를 분석하고, 
2단계에서 MAICE 시스템과 QAC 루브릭을 설계·구축하였다. 3단계는 고2 학생 58명을 무작위로 
Agent 모드와 Freepass 모드에 배정하여 2주간 현장 실험을 수행하였다. 4단계에서는 AI 3개 모델, 
교사 2명, 학생 설문의 다각도 평가를 실시하였다. 5단계에서 통계 분석, 신뢰도 검증, 
질적 분석을 통해 삼각검증을 수행하였으며, 6단계에서 설계 원리를 도출하였다. DBR의 반복적 
특성에 따라 도출된 원리는 시스템 개선에 환류된다.
```
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/figure6-1-research-design.svg}
    \caption{Design-Based Research(DBR) 기반 연구 설계 다이어그램}
