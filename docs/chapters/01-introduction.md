...
본 연구에서는 MAICE 시스템 설계에 앞서, 현재 프리패스 방식 LLM의 교육적 문제점을 실증적으로 파악하기 위해 2024년 5월 중순(학기 초)에 예비 조사를 실시하였다. 예비 조사는 다음의 순차적 과정으로 진행되었다:
...
**2단계: 문제점 파악 및 루브릭 구성** - 수집된 데이터를 분석한 결과, 학생 질문과 AI 답변에서 반복적으로 나타나는 질적 문제 패턴들이 관찰되었다. 이러한 문제점들을 명료화하고 그 존재 여부를 객관적으로 판단하기 위해, 최신 AI 교육 연구의 성과를 종합하여 체계적인 분석적 채점 기준(analytic rubric)을 개발하였다.

루브릭 설계는 다음의 이론적 기반 위에서 이루어졌다:

첫째, **학생 질문 평가 영역**은 관찰된 구체적 문제점을 체계화하는 과정에서 도출되었다. 수학과 무관한 질문(45.5% 최저점)에 대응하여 **수학적 전문성** 영역을, 질문 의도 파악 불가 문제(45.8% 최저점)에 대응하여 **질문 구조화** 영역을, 맥락 정보 부재 문제(72.3% 최저점)에 대응하여 **학습 맥락 적용** 영역을 구성하였다. 이는 효과적인 질문 형성의 교육학적 원리(Rothstein & Santana, 2011)와 메타인지 전략(Zimmerman, 2002)에 이론적 기반을 두고 있다.

둘째, **AI 답변 평가 영역**은 최신 AI 교육 연구의 평가 프레임워크를 통합하여 설계되었다. **학습자 맞춤도** 영역은 Maurya et al.(2024)의 MRBench에서 제시한 "Actionability(실행가능성)" 차원—학습자에게 다음 단계가 명확한지 평가—을 반영하여, AI가 학습자 수준을 파악하고 선수지식을 연계하며 개인화된 피드백을 제공하는지 평가한다. **설명의 체계성** 영역은 MRBench의 "Coherence(일관성)"와 "Providing Guidance(안내 제공)" 차원을 결합하고, Pauzi et al.(2025)의 UCL 소크라테스 루브릭에서 강조한 "비계 설정(Scaffolding)" 원리를 반영하여, 개념의 위계적 설명과 단계별 논리 전개를 평가한다. **학습 내용 확장성** 영역은 Schmucker et al.(2024)의 Ruffle&Riley 연구에서 측정한 학습 경험 차원(Understanding, Helpfulness)과 소크라테스 루브릭의 "Guidance and Actionability" 원리를 통합하여, 심화학습 방향 제시와 자기주도 학습 유도를 평가한다.

이러한 이론적 기반 위에서 개발된 루브릭은 질문 평가 3개 영역과 AI 답변 평가 3개 영역으로 구성되었으며, 각 영역은 5점 척도로 평가된다. 이는 MRBench의 3단계 척도를 확장하고, RUBICON(Biyani et al., 2024)의 도메인 특화 평가 접근법을 수학 교육에 적용한 것이다. 나아가 본 연구는 기존 연구들이 주로 AI 응답 품질에 초점을 맞춘 것과 달리, 학생 질문의 품질을 독립적으로 평가하고 질문 명료화 프로세스의 필요성을 실증적으로 입증함으로써 독창적인 기여를 하였다.

**3단계: 동료교사 검증** - 개발된 루브릭의 타당성과 신뢰도를 검증하기 위해, 중등 수학교사 4명(평균 교직 경력 8.5년)으로 구성된 전문가 패널이 사전 교육을 받은 후 독립적으로 385건의 질문-답변을 평가하였다(총 1,012건의 교사 평가). 교사 간 평가 일치도 분석 결과, 특히 본 연구의 핵심인 학습맥락 영역에서 평균 표준편차 0.517점(5점 척도 대비 10.3%)으로 매우 높은 일관성을 보였으며, 모든 영역에서 수용 가능한 수준의 신뢰도를 확보하였다.

**4단계: 문제점 특정 및 에이전트 설계** - 교사 루브릭 평가 결과를 통해 프리패스 방식의 구체적 문제점들(학습맥락 부재 72.3%, 학습확장성 결여 48.9%, 질문-답변 품질 상관관계 r=0.691)이 실증적으로 특정되었으며, 이 분석 결과가 MAICE의 질문 명료화 기반 멀티 에이전트 시스템 설계의 핵심 근거가 되었다.
...
추가 분석 결과, 질문 품질과 답변 품질 간에는 강한 양의 상관관계(r=0.691)가 확인되어, 질문이 명료하고 맥락이 풍부할수록 AI 답변의 질도 향상되는 것으로 나타났다. 특히 질문의 학습맥락 점수와 답변의 학습확장성 점수 간 상관계수는 0.521로, 학생이 자신의 학습 상황을 명확히 제시할수록 AI가 학습을 확장하는 답변을 제공할 가능성이 높아짐을 보여준다. **이는 질문 명료화 프로세스를 핵심 전략으로 하는 MAICE 시스템 개발의 실증적 근거가 되었다.**

이러한 문제점들은 단순히 개별 사례의 오류가 아니라, 현재 LLM이 채택하고 있는 프리패스 방식의 구조적 한계에서 비롯된다. 프리패스 방식은 질문의 질이나 맥락과 무관하게 즉각적인 답변을 제공함으로써, 학습자가 스스로 질문을 명료화하고 구조화하는 과정을 경험할 기회를 제공하지 않는다. 예비 조사에서 관찰된 바와 같이, 이는 학생들의 LLM 활용 만족도를 저하시킬 뿐만 아니라, 실질적인 학습 효과를 얻지 못하는 결과를 초래한다.
...
따라서 예비 조사에서 확인된 프리패스 방식의 구조적 한계를 극복하고, 질문 명료화 과정을 통해 학습자의 사고 과정을 깊이 있게 지원하며, 교사 루브릭 평가에서 드러난 문제점들(학습맥락 부재 72.3%, 학습확장성 결여 48.9%)을 개선할 수 있는 **질문 명료화 기반 AI agent 시스템 MAICE의 개발이 필요하다**. 본 연구는 이러한 필요성에 기반하여 MAICE 시스템을 설계하고 개발하며, 실제 수업 환경에서 프리패스 방식 대비 우수한 학습 효과를 입증하는 것을 목적으로 한다.

### 1.1.5 MAICE 평가를 위한 QAC 체크리스트

예비 조사에서 개발된 6개 영역 루브릭(각 5점)은 프리패스 방식의 문제점을 진단하는 데 효과적이었으나, MAICE의 질문 명료화 프로세스와 멀티 에이전트 시스템의 교육적 효과를 종합적으로 평가하기 위해서는 더 세밀한 평가 체계가 필요하였다. 이에 본 연구는 예비 조사 루브릭을 기반으로 **대화 맥락 평가 영역**을 추가하고, 각 주요 영역을 3개의 세부 항목으로 분해한 **QAC 체크리스트(Question-Answer-Context Checklist, 9개 세부 항목, 45점 만점)**를 개발하였다.

QAC 체크리스트의 구조는 다음과 같다:

**A. 학생 질문 평가 (15점 만점)**
- A1. 수학적 전문성 (5점): 수학 학습 목적 부합도, 수학적 개념·원리·절차의 정확성
- A2. 질문 구조화 (5점): 질문 대상·범위·초점의 명확성, 구체적 어려움 제시
- A3. 학습 맥락 적용 (5점): 학습자 수준·선수지식·학습 목적·이해 상태 명시

**B. AI 응답 평가 (15점 만점)**
- B1. 학습자 맞춤도 (5점): 학습자 수준 파악, 선수지식 연계, 개인화된 피드백
- B2. 설명의 체계성 (5점): 개념의 위계적 설명, 단계별 논리 전개, 핵심 강조
- B3. 학습 내용 확장성 (5점): 심화학습 방향 제시, 응용문제 연계, 자기주도 학습 유도

**C. 대화 맥락 평가 (15점 만점)** - 신규 추가
- C1. 질문 명료화 효과성 (5점): 명료화 지원이 질문 개선에 기여한 정도
- C2. 대화 일관성 및 연속성 (5점): 학습 목표 중심의 일관된 대화 진행, 이전 맥락 활용
- C3. 학습 과정 지원성 (5점): 사고 과정 유도, 이해 점검, 메타인지 활동 촉진

대화 맥락 평가 영역(C)은 MAICE의 핵심 차별화 요소인 질문 명료화 프로세스의 효과성을 측정하기 위해 새롭게 추가되었다. C1(질문 명료화 효과성)은 MRBench의 "Mistake Identification" 차원과 소크라테스 루브릭의 대화법 원리를 결합하여, 프리패스 방식의 즉시 답변에서 명료화 중심 접근으로의 패러다임 전환을 평가한다. C2(대화 일관성)는 Schmucker et al.(2024)의 상호작용 분석 지표와 소크라테스 루브릭의 "Coherence and Tone"을 반영하여 멀티 에이전트 시스템의 대화 관리 능력을 측정한다. C3(학습 과정 지원성)은 소크라테스 루브릭의 "Scaffolding"과 Ruffle&Riley의 "Engagement" 차원을 통합하여, 단순 정보 제공을 넘어 학습자의 사고 과정을 깊이 있게 지원하는지 평가한다.

QAC 체크리스트는 MAICE 시스템의 효과성을 다각도로 평가할 수 있는 종합적인 평가 도구로, 본 연구의 모든 실증 분석에 활용되었다. 특히 질문 품질 개선(RQ1), 학습 경험 향상(RQ2), 학습 효과 개선(RQ3)을 각각 A, C, B 영역을 통해 독립적으로 측정할 수 있도록 설계되었다.

## 1.2 연구 목적

예비 조사에서 확인된 핵심 발견은 질문 품질과 답변 품질 간 강한 상관관계(r=0.691)로, 이는 **질문 개선이 학습 효과 향상의 핵심 메커니즘**임을 의미한다. 따라서 본 연구는 고등학교 2학년 수학적 귀납법 단원을 중심으로 **질문 명료화 프로세스를 핵심으로 하는 AI agent 시스템 MAICE(Mathematical AI Chatbot for Education)를 설계·개발하고, 질문 품질 개선을 통한 학습 효과 향상을 실증적으로 입증**하는 데 목적이 있다. 구체적인 연구 목적은 다음과 같다:
...
2. **질문 품질 개선 효과 검증**: MAICE의 질문 명료화 프로세스가 학생의 질문 품질(학습맥락, 질문구조, 수학적전문성)을 프리패스 방식 대비 실질적으로 개선함을 입증

3. **학습 경험 향상 증명**: 질문 명료화 프로세스를 통한 구조화된 학습 경험이 즉시 답변 방식보다 학습 만족도와 메타인지를 향상시킴을 입증

4. **학습 효과 개선 검증**: 질문 품질 개선이 실제 학습 성과로 이어지는지 확인하기 위해, 수학적 귀납법 학습 효과(이해도, 문제해결 능력)에서 프리패스 방식 대비 우수함을 검증

5. **수학적 귀납법 단원 맞춤 설계**: 수학적 귀납법의 학습 특성(논리 구조 이해, 단계별 증명)을 고려한 단원별 맞춤형 agent 설계

## 1.3 연구 문제

예비 조사를 통해 확인된 프리패스 방식의 핵심 문제는 학생들의 질문 품질 부족(학습맥락 72.3% 최저점)이며, 질문 품질과 답변 품질 간 강한 상관관계(r=0.691)는 질문 개선이 학습 효과 향상의 핵심 메커니즘임을 시사한다. 따라서 MAICE 시스템의 효과성을 검증하기 위한 연구 문제를 다음과 같이 설정한다:

**RQ1. 질문 품질 개선 검증 (핵심 메커니즘)**: MAICE의 질문 명료화 프로세스가 학생의 질문 품질을 실질적으로 개선하는가? 특히 예비 조사에서 가장 심각한 문제로 확인된 학습 맥락 적용, 질문 구조화, 수학적 전문성 영역에서 프리패스 방식 대비 유의미한 개선이 나타나는가?

**RQ2. 학습 경험 향상 검증 (교육적 가치)**: 질문 명료화 프로세스를 통한 구조화된 학습 경험이 즉시 답변 방식보다 학생의 학습 만족도와 메타인지를 향상시키는가?

**RQ3. 학습 효과 향상 검증 (부가적 성과)**: 질문 품질 개선과 학습 경험 향상이 실제 학습 성과로 이어지는가? MAICE 시스템이 프리패스 방식보다 수학적 귀납법 학습 효과(이해도, 문제해결 능력)에서 우수한가?
...
- **MAICE (Mathematical AI Chatbot for Education)**: 예비 조사에서 확인된 프리패스 방식의 한계를 극복하기 위해 개발된 질문 명료화 기반 AI agent 시스템
