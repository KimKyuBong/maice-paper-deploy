...
각 영역은 **5점 리커트 척도(1=매우 부족 ~ 5=매우 우수)**로 평가되며, 교사가 6개 영역 각각에 대해 전체적인 판단을 바탕으로 1~5점을 직접 부여하였다. 평가를 돕기 위해 각 영역별로 4개의 세부 평가 요소(총 24개)를 참고 가이드로 제시하였으나, 최종 점수는 교사의 종합적 판단에 따랐다. 질문 영역 15점, 답변 영역 15점으로 총 30점 만점이다.
...
**3단계: 동료교사 검증 및 루브릭 개선** - 중등 수학교사 4명(교직 경력: 1년차 2명, 5년차 1명, 7년차 1명, 평균 3.5년)이 독립적으로 385건의 질문-답변을 6개 영역 각각에 대해 1~5점으로 평가하였다. 교사 간 상관관계는 평균 r=0.28, ICC(2,k)=0.29로 나타났으며, 특히 수학적 전문성과 학습확장성 영역에서 평가 차이가 컸다(r=0.01~0.26). 이러한 낮은 일치도는 교사의 종합적 판단에 의한 점수 부여 방식의 주관성에서 비롯되었으며, 이를 개선하기 위해 **각 영역을 4개의 명확한 체크리스트 요소로 분해하고 이진 판단(0/1)으로 평가하는 QAC 체크리스트 방식(6.4절)으로 전환**하였다.

**4단계: 주요 발견 및 에이전트 설계** - 교사 루브릭 평가 결과, 프리패스 방식의 구체적 문제점들이 실증적으로 확인되었다: (1) 학습맥락 부재 72.3%, (2) 학습확장성 결여 48.9%, (3) **질문 품질과 답변 품질 간 강한 상관관계 r=0.691**. 특히 세 번째 발견은 **질문 개선이 학습 효과 향상의 핵심 메커니즘**임을 시사하며, 이는 MAICE의 질문 명료화 기반 멀티 에이전트 시스템 설계의 핵심 근거가 되었다.
...
예비 조사 루브릭(6개 영역, 5점 척도)의 교사 간 낮은 일치도(r=0.28)를 개선하고, MAICE의 교육적 효과를 더 객관적으로 측정하기 위해, 본 연구는 **QAC 체크리스트(Question-Answer-Context Checklist)**를 개발하였다. QAC는 예비조사 루브릭에 **대화 맥락 평가 영역(C)**을 추가하고, 각 항목을 4개의 체크리스트 요소로 구체화한 평가 도구이다(8개 항목, 32개 체크리스트, 40점 만점).

**QAC 체크리스트의 구조:**

**A. 학생 질문 평가 (15점)**: A1. 수학적 전문성, A2. 질문 구조화, A3. 학습 맥락 적용

**B. AI 응답 평가 (15점)**: B1. 학습자 맞춤도, B2. 설명의 체계성, B3. 학습 내용 확장성

**C. 대화 맥락 평가 (10점)**: C1. 대화 일관성 및 연속성, C2. 학습 과정 지원 (신규 추가)

각 항목은 4개 체크리스트로 평가되며(0/1 이진 판단), 충족 개수로 자동 점수 산정된다(0개=1점 ~ 4개=5점). C 영역은 멀티 에이전트 시스템의 대화 관리 능력과 학습 지원을 측정하며, Pass 모드와 Agent 모드 간 공정한 비교를 가능하게 한다.

(QAC 체크리스트의 상세 구성과 AI 채점 타당성 검증 방법은 6.4절 참조, 전체 체크리스트는 부록 A 참조)

## 1.2 연구 목적

예비 조사에서 확인된 핵심 발견은 질문 품질과 답변 품질 간 강한 상관관계(r=0.691)로, 이는 **질문 개선이 학습 효과 향상의 핵심 메커니즘**임을 의미한다. 따라서 본 연구는 고등학교 2학년 수학적 귀납법 단원을 중심으로 **질문 명료화 프로세스를 핵심으로 하는 AI agent 시스템 MAICE(Mathematical AI Chatbot for Education)를 설계·개발하고, 질문 품질 개선을 통한 학습 효과 향상을 실증적으로 입증**하는 데 목적이 있다. 
...
본 시스템은 학술적 검증뿐만 아니라 교육 현장에서의 실제 활용을 고려하여, 교사가 자율적으로 커스터마이징하고 확장할 수 있는 범용적 구조로 설계하였다.
