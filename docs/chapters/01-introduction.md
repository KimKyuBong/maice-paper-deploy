...
예비 조사에서 관찰된 구체적 문제 패턴에 대응하여 6개 평가 영역을 구성하였다:

**질문 평가 영역 (학생의 질문 품질)**:
- **수학적 전문성 (A1)**: 45.5%가 최저점을 받은 "수학과 무관한 질문" 문제 → 수학 개념의 정확성, 교과과정 내 위치, 용어 사용의 적절성 평가
- **질문 구조화 (A2)**: 45.8%가 최저점을 받은 "모호하고 불명확한 질문" 문제 → 질문 대상·범위·초점의 명확성, 구체적 어려움 제시 평가
- **학습 맥락 적용 (A3)**: 72.3%가 최저점을 받은 "맥락 정보 부재" 문제 (가장 심각) → 학습자 수준, 선수지식, 학습 목적, 이해 상태 명시 평가

**AI 답변 평가 영역 (AI의 교육적 응답 품질)**:
- **학습자 맞춤도 (B1)**: 평균 2.474점(27.6% 최저점)의 "학습자 수준 파악 실패" 문제 → 학습자 수준 파악, 선수지식 연계, 난이도 조절, 개인화된 피드백 평가
- **설명의 체계성 (B2)**: 평균 2.765점(23.9% 최저점)의 "비체계적 설명" 문제 → 개념의 위계적 설명, 단계별 논리 전개, 핵심 요소 강조, 예시 활용 평가
- **학습 내용 확장성 (B3)**: 평균 1.832점(48.9% 최저점)의 "심화학습 유도 실패" 문제 (AI 답변 중 가장 심각) → 심화학습 방향 제시, 응용문제 연계, 오개념 교정, 자기주도 학습 유도 평가

각 영역은 5점 리커트 척도(1=매우 부족 ~ 5=매우 우수)로 평가되며, 각 영역별로 4개의 세부 평가 요소를 구성하여 총 24개 세부 항목으로 측정한다. 질문 영역 15점, 답변 영역 15점으로 총 30점 만점으로 구성된다.

**루브릭의 독창적 기여**: 본 루브릭은 기존 AI 교육 평가 연구(MRBench, RUBICON, 소크라테스 루브릭)가 주로 AI 응답 품질에 초점을 맞춘 것과 달리, (1) 학생 질문의 품질을 독립적으로 평가하고, (2) 실제 교실 데이터 분석을 통해 관찰된 문제를 직접 해결하며, (3) 전통적 교육학 이론과 최신 AI 평가 연구를 통합하고, (4) 질문 품질과 답변 품질 간 강한 상관관계(r=0.691)를 실증하여 질문 명료화가 학습 효과 향상의 핵심 메커니즘임을 입증하였다.

(루브릭 개발의 상세한 이론적 기반과 각 문항 구성 근거는 2장 이론적 배경 및 부록 A 참조)

**3단계: 동료교사 검증** - 개발된 루브릭의 문제점을 식별하고 개선점을 도출하기 위해, 중등 수학교사 3명(평균 교직 경력 8.5년)이 독립적으로 385건의 질문-답변을 평가하였다. 이 단계는 루브릭의 신뢰도 검증보다는 평가 기준의 모호성과 해석 차이를 발견하는 데 초점을 맞추었다. 분석 결과, 교사 간 상관관계는 평균 r=0.28, ICC(2,k)=0.29로 낮게 나타났으며, 이는 예상된 결과로서 루브릭 기준의 명확화가 필요함을 시사하였다. 특히 질문의 수학적 전문성과 응답의 학습확장성 영역에서 교사 간 평가 차이가 컸으며(r=0.01~0.26), 이를 바탕으로 루브릭을 세밀하게 수정하였다.
...
예비 조사에서 개발된 6개 영역 루브릭(각 5점)은 프리패스 방식의 문제점을 진단하는 데 효과적이었으나, MAICE의 질문 명료화 프로세스와 멀티 에이전트 시스템의 교육적 효과를 종합적으로 평가하기 위해서는 더 세밀하고 객관적인 평가 체계가 필요하였다. 이에 본 연구는 예비 조사 루브릭을 기반으로 **대화 맥락 평가 영역**을 추가하고, 각 주요 영역을 세부 항목으로 분해한 후, 각 항목을 4개의 체크리스트 요소로 구체화한 **QAC 체크리스트(Question-Answer-Context Checklist, 8개 항목, 32개 체크리스트 요소, 40점 만점)**를 개발하였다.

**체크리스트 방식의 특징**:
- **객관적 평가**: 각 요소를 0(미충족)/1(충족)로 이진 판단하여 주관성 최소화
- **투명한 근거**: 각 요소별 평가 근거(evidence) 명시
- **자동 점수 산정**: 충족 요소 개수 + 1 (0개=1점, 1개=2점, 2개=3점, 3개=4점, 4개=5점)
- **공정한 비교**: C1(명료화 효과성) 제외로 Agent 방식 편향 제거, 대화 방식과 무관하게 교육적 효과만 평가

QAC 체크리스트의 구조는 다음과 같다:

**A. 학생 질문 평가 (15점 만점, 12개 체크리스트 요소)**
- A1. 수학적 전문성 (5점): 개념 정확성, 교과과정 위계성, 용어 적절성, 문제해결 방향 구체성
- A2. 질문 구조화 (5점): 질문 단일성, 조건 완결성, 문장 논리성, 의도 명시성
- A3. 학습 맥락 적용 (5점): 현재 학습단계 설명, 선수학습 언급, 구체적 어려움 명시, 학습목표 제시

**B. AI 응답 평가 (15점 만점, 12개 체크리스트 요소)**
- B1. 학습자 맞춤도 (5점): 수준별 접근, 선수지식 연계, 난이도 조절, 개인화 피드백
- B2. 설명의 체계성 (5점): 개념 위계화, 단계별 논리 전개, 핵심 강조, 예시 활용 적절성
- B3. 학습 내용 확장성 (5점): 심화방향 제시, 응용문제 연계, 오개념 교정, 자기주도 학습 유도

**C. 대화 맥락 평가 (10점 만점, 8개 체크리스트 요소)** - 신규 추가
- C1. 대화 일관성 및 연속성 (5점): 학습목표 중심 일관성, 이전 맥락 참조, 주제 연속성, 이전 턴 연결성
- C2. 학습 과정 지원성 (5점): 사고과정 유도, 이해도 확인, 메타인지 촉진, 깊이있는 사고 유도


대화 맥락 평가 영역(C)은 MAICE의 교육적 효과를 대화 방식과 무관하게 공정하게 측정하기 위해 새롭게 추가되었다. C1(대화 일관성 및 연속성)은 Schmucker et al.(2024)의 상호작용 분석 지표와 소크라테스 루브릭의 "Coherence and Tone"을 반영하여 멀티 에이전트 시스템의 대화 관리 능력을 측정한다. C2(학습 과정 지원성)는 소크라테스 루브릭의 "Scaffolding"과 Ruffle&Riley의 "Engagement" 차원을 통합하여, 단순 정보 제공을 넘어 학습자의 사고 과정을 깊이 있게 지원하는지 평가한다. 이를 통해 Pass 모드와 Agent 모드 간 교육적 결과만을 공정하게 비교할 수 있도록 설계되었다.
