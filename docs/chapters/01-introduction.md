...
### 1.1.5 MAICE 평가를 위한 확장 루브릭

예비 조사에서 개발된 6개 영역 루브릭(각 5점)은 프리패스 방식의 문제점을 진단하는 데 효과적이었으나, MAICE의 질문 명료화 프로세스와 멀티 에이전트 시스템의 교육적 효과를 종합적으로 평가하기 위해서는 더 세밀한 평가 체계가 필요하였다. 이에 본 연구는 예비 조사 루브릭을 기반으로 **대화 맥락 평가 영역**을 추가하고, 각 주요 영역을 3개의 세부 항목으로 분해한 **9개 세부 항목, 45점 만점의 확장 루브릭**을 개발하였다.
...
이러한 확장 루브릭은 MAICE 시스템의 효과성을 다각도로 평가할 수 있는 종합적인 평가 도구로, 본 연구의 모든 실증 분석에 활용되었다. 특히 질문 품질 개선(RQ1), 학습 경험 향상(RQ2), 학습 효과 개선(RQ3)을 각각 A, C, B 영역을 통해 독립적으로 측정할 수 있도록 설계되었다.
