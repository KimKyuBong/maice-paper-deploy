...
This study employed Design-Based Research (DBR) combined with a quasi-experimental design. Fifty-eight high school grade 2 students from Busan Metropolitan City were randomly assigned to either Agent mode (clarification process provided, n=28) or Freepass mode (immediate answers provided, n=30). The study was conducted over approximately two weeks from October 20 to November 1, 2025, utilizing A/B testing methodology.

The QAC Checklist (Question-Answer-Context Checklist, 40 points, 8 sub-items) was developed to evaluate question quality (mathematical professionalism, question structure, learning context), answer quality (learner customization, explanation systematicity, learning expandability), and dialogue context (dialogue consistency, learning support). Three independent AI models (Gemini, Claude, GPT-5) were used as raters, analyzing 221 common sessions with inter-rater reliability of ICC(2,1)=0.595 and Cronbach's α=0.840.
