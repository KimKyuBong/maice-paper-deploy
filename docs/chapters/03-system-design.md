...
### 가. Classifier Agent: "이 질문은 어떤 유형인가?"
...
### 나. Question Improvement Agent: "질문을 더 명확하게"
...
### 다. Answer Generator Agent: "유형별 맞춤 답변"
...
### 라. Observer Agent: "대화 요약 및 컨텍스트 관리"
...
### 마. FreeTalker Agent: "대조군 (Freepass 모드)"
...
베타테스트 초기 버전에서 **시스템이 대화 맥락을 유지하지 못하는 문제**가 발생하였다. 학생이 이전 질문과 연결된 추가 질문을 했을 때, AI가 이전 대화를 기억하지 못해 답변이 단절되는 현상이었다.
...
2. ✅ **수식 입력 가이드**: 수식 버튼 옆에 도움말 툴팁 표시
   - "💡 수식 입력: 버튼 클릭 또는 이미지 업로드"
   - "📷 이미지 OCR: 문제 사진을 찍어 바로 질문"
