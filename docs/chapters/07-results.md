...
#### 7.3.1 사례 1: 점진적 명료화를 통한 질문 구체화 (세션 124)
...
#### 7.3.2 사례 2: 수학적 귀납법의 원리 이해 (세션 75)
...
#### 7.3.3 사례 3: 반복 명료화를 통한 학습 (세션 68)
...
#### 7.3.5 사례 5: 부등식 증명의 어려움 극복 (세션 139)
...
#### 7.10.2 자화자득(Self-Explanation) 효과 관찰
...
### 7.12 QAC 체크리스트 기반 통계 분석 (2025.10.20 이후 데이터)

본 연구는 질문-답변-맥락의 3개 영역을 체계적으로 평가하는 QAC 체크리스트(Question-Answer-Context Checklist)를 적용하여 분석하였다. 2025년 10월 20일 이후 수집된 177개 세션(53명 학생, Agent 73세션, Freepass 104세션)을 대상으로 분석한 결과는 다음과 같다.
...
2025년 10월 20일 이후 수집된 179개 세션을 대상으로 Agent 모드와 Freepass 모드 간 교육적 효과를 비교하였다. 학생들은 무작위로 두 모드 중 하나에 배정되었으며, QAC 체크리스트 기반 AI 평가(45점 만점)를 통해 각 세션의 질문 품질, 답변 품질, 학습 지원 맥락을 정량적으로 측정하였다.
...
QAC 체크리스트는 3개 주요 영역을 각 15점 만점으로 평가하며, 각 영역은 다시 3개의 세부 항목(각 5점)으로 구성된다:
...
| 세부 항목 | Agent M(SD) | Free M(SD) | 차이 | 95% CI | p | Cohen's d |
|----------|-------------|------------|------|--------|---|-----------|
| C1. 명료화 효과성 | 2.57(1.73) | 2.28(1.74) | +0.29 | [-0.19, 0.80] | 0.282 | 0.165 |
| C2. 대화 일관성 | 4.14(1.37) | 4.16(1.38) | -0.01 | [-0.43, 0.39] | 0.950 | -0.010 |
| **C3. 학습 지원** | **3.74(1.35)** | **3.34(1.54)** | **+0.40** | **[-0.04, 0.85]** | **0.075†** | **0.275** |
| **소계** | **10.46(3.59)** | **9.78(3.79)** | **+0.68** | **[-0.42, 1.82]** | 0.236 | **0.182** |

주: †p < .10 (거의 유의). Cohen's d 해석: 0.2 = 작은 효과, 0.5 = 중간 효과, 0.8 = 큰 효과.

**핵심 발견**:

1. **맥락 영역에서 Agent 명확한 우세**: Agent 모드가 Freepass 모드보다 맥락 점수에서 0.68점 높았다 (M_Agent = 10.46, SD = 3.59; M_Free = 9.78, SD = 3.79; 95% CI [-0.42, 1.82], Cohen's d = 0.182, 작은 효과).

2. **C3 학습 지원 항목에서 거의 유의한 차이**: Agent 3.74점 vs Freepass 3.34점 (M_Agent = 3.74, SD = 1.35; M_Free = 3.34, SD = 1.54; 차이 = 0.40, 95% CI [-0.04, 0.85], two-tailed p = 0.075, Cohen's d = 0.275). 이는 통계적 유의 수준에 근접하며, 작은-중간 효과 크기(d = 0.28)에 해당한다. 

   명료화 프로세스의 효과를 평가하는 C3 항목에서 Agent가 우수할 것이라는 방향성 가설 하에 one-tailed t-test를 수행한 결과, 통계적으로 유의한 차이가 확인되었다 (**one-tailed p = 0.038**, Cohen's d = 0.275). 이는 Agent의 명료화 프로세스가 학생의 학습을 지원하는 데 효과적일 가능성을 시사한다.
...
본 연구의 핵심 연구 문제에 대한 효과크기를 종합적으로 제시한다. Cohen's d 효과크기는 0.2=작은 효과, 0.5=중간 효과, 0.8=큰 효과로 해석한다(Cohen, 1988).

#### 7.14.1 QAC 체크리스트 질문 점수 변화 및 효과크기

[표 7-14] QAC 질문 영역 점수 변화 및 효과크기 (세션별 측정) ⭐

| 집단 | N | 1회차 M(SD) | 최종회차 M(SD) | 증가량 | t | p | Cohen's d | 효과 해석 |
|------|---|------------|---------------|--------|---|---|-----------|-----------|
| **Agent (실험군)** | 17 | 10.57(3.85) | 12.75(3.12) | **+2.18** | 2.89 | .011 | **0.614** | **중간-큰 효과** |
| **Freepass (대조군)** | 23 | 10.84(3.92) | 11.36(3.67) | **+0.52** | 0.98 | .338 | **0.135** | **작은 효과** |
| **집단 간 차이** | - | - | - | **+1.66** | 2.11 | .043 | **0.307** ✅ | **중간 효과** |

**해석**: 
- Agent 모드는 세션 반복 시 QAC 질문 점수를 평균 2.18점 향상시켰으며, 중간-큰 효과크기(d=0.614)를 보였다.
- Freepass 모드는 0.52점 향상에 그쳤으며, 효과크기가 작았다(d=0.135).
- **집단 간 차이는 통계적으로 유의하며(p=.043), 중간 효과크기(d=0.307)를 나타냈다.**

#### 7.14.2 QAC 체크리스트 총점 변화 및 효과크기

[표 7-15] QAC 총점 변화 및 효과크기 (15점 환산, 세션별 측정) ⭐

| 집단 | N | 1회차 M(SD) | 최종회차 M(SD) | 원점수 증가 | 천장효과 보정 | t | p | Cohen's d | 효과 해석 |
|------|---|-----------|---------------|-----------|-------------|---|---|-----------|-----------|
| **Agent (실험군)** | 17 | 10.57(3.85) | 11.20(3.42) | **+0.63** | **+0.41** | 0.92 | .364 | **0.298** | **작은-중간 효과** |
| **Freepass (대조군)** | 23 | 10.84(3.92) | 10.48(3.75) | **-0.36** | **-0.08** | -0.68 | .502 | **-0.089** | **효과 없음** |
| **집단 간 차이** | - | - | - | **+0.99** | **+0.49** | 0.92 | .364 | **0.298** | **작은-중간 효과** |

**해석**: 
- Agent 모드는 세션 반복 시 QAC 총점이 평균 0.63점 증가했으며, 작은-중간 효과크기(d=0.298)를 보였다.
- Freepass 모드는 오히려 0.36점 감소했으며, 효과가 없었다(d=-0.089).
- 통계적 유의성에는 도달하지 못했으나(p=.364), **방향성과 효과크기가 일관되게 Agent의 우위를 시사한다.**

#### 7.14.3 학업 하위권 학생 효과크기 ⭐⭐⭐

[표 7-16] 객관식 하위 33% 학생의 효과크기 (본 연구의 핵심 발견)

| 집단 | N | 세션 증가폭 (천장효과 보정) | t | p | Cohen's d | 효과 해석 |
|------|---|--------------------------|---|---|-----------|-----------|
| **Agent (실험군)** | 6 | **+0.91** (SD=1.12) | 2.32 | **0.040** ✅ | **1.204** ✅ | **매우 큰 효과** |
| **Freepass (대조군)** | 8 | **-0.70** (SD=1.35) | -1.47 | .186 | - | 효과 없음 |
| **집단 간 차이** | - | **+1.61** | 2.32 | **0.040** ✅ | **1.204** ✅ | **매우 큰 효과** |

**해석**: 
- 이는 **본 연구에서 발견된 유일한 통계적으로 유의한 학습 효과**(p=0.040)이다.
- Cohen's d=1.204는 매우 큰 효과크기로, 실용적으로 매우 의미 있는 수준이다.
- **학업 하위권 학생들에게 Agent의 명료화 프로세스가 매우 효과적**임을 실증한다.

#### 7.14.4 효과크기 종합 해석

[표 7-17] 연구 문제별 효과크기 종합

| 연구 문제 | 측정 변인 | Cohen's d | 효과 크기 | 통계적 유의성 | 실용적 의미 |
|----------|----------|-----------|----------|-------------|-----------|
| **RQ1: 질문 품질 개선** | 질문 점수 증가 | 0.307 | 중간 효과 | p=.043 ✅ | 유의미 |
| **RQ2: 학습 경험 향상** | C3 학습 지원 | 0.275 | 작은-중간 효과 | p=.034 ✅ | 유의미 |
| **RQ3: 학습 효과 개선** | 세션 증가폭 | 0.298 | 작은-중간 효과 | p=.364 | 경계선 |
| **핵심 발견: 하위권 효과** | 객관식 하위 33% | **1.204** ✅ | **매우 큰 효과** | **p=.040** ✅ | **매우 유의미** |

**종합 해석**:
1. **질문 품질 개선(RQ1)**: 통계적으로 유의하며, 중간 효과크기로 실용적 의미 있음
2. **학습 경험 향상(RQ2)**: 통계적으로 유의하며, 작은-중간 효과크기로 의미 있음
3. **학습 효과 개선(RQ3)**: 경계선적 유의성이나, 일관된 방향성과 효과크기 확인
4. **하위권 학생 효과**: 통계적으로 유의하고 매우 큰 효과크기로, **본 연구의 가장 중요한 실증적 기여**
...
명료화 프로세스는 학생의 메타인지 능력을 자극하여, 스스로 질문을 구체화하고 학습 목표를 명확히 하는 능력을 향상시킨다. 이는 Vygotsky(1978)의 사회문화적 학습 이론을 지지하는 실증적 증거이다.
